...
namespace Game
{
    ...
    class AddressablesLoader : SAssetLoader
    {
        ...
        {
            var wait = Addressables.InstantiateAsync(url, parent: GameM.World.transform);
            wait.WaitForCompletion();
            return wait.Result;
        }

        public override STask<GameObject> LoadGameObjectAsync(string url) => Addressables.InstantiateAsync(url, parent: GameM.World.transform).AsTask();
