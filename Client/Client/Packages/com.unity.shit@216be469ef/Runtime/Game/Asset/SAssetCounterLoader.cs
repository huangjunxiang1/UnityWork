...
namespace Game
{
    ...
    public class SAssetCounterLoader : SAssetBaseLoader
    {
        class Temp
        {
        ...
        public override UnityEngine.Object Load(string path)
        {
            if (!counter.TryGetValue(path, out Temp value))
            {
                value = new Temp();
                counter[path] = value;
                value.wait = Addressables.LoadAssetAsync<UnityEngine.Object>(SAsset.Directory + path);
        ...
        public override STask<UnityEngine.Object> LoadAsync(string path)
        {
            STask<UnityEngine.Object> task = new(path);
            if (counter.TryGetValue(path, out Temp value))
            {
                value.count++;
        ...
        async void getTaskAndWait(string path, STask<UnityEngine.Object> task)
        {
            if (!counter.TryGetValue(path, out Temp value))
            {
                value = new Temp();
