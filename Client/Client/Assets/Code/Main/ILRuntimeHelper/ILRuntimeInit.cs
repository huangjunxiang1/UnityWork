...
public unsafe static class ILRuntimeInit
{
    ...
    static void log(string condition, string stackTrace, LogType type)
    {
        if (condition.Contains("Please add following code:"))
        {
            ...
            for (int i = 0; i < s.Length; i++)
            {
                ...
                {
                    string path = Application.dataPath + "/Code/Main/ILRuntimeHelper/ILRuntimeInit.cs";
                    ...
                    if (s[i].Contains("Cannot find Delegate Adapter"))
                    {
                        ...
                        {
                            if (item.Contains(ctx))
                                return;
                        }

                        int idx = cs.IndexOf("        //xx1");
                        if (idx == -1)
                        {
                    ...
                    else if (i > 0 && s[i - 1].Contains("Cannot find convertor"))
                    {
                        ...
                        int idx = cs.IndexOf("        //xx2");
                        if (idx == -1)
                        {
                            Loger.Error("autoGenerateDelegateAdapter Cant find markLine");
                            return;
                        }
                        cs.Insert(idx + 1, "        " + s[i + 7]);
                        cs.Insert(idx + 1, "        " + s[i + 6]);
                        string ss = s[i + 5];
                        if (ss.IndexOf("<") + 1 == ss.IndexOf(">"))
                            ss = ss.Replace("<>", null);
                        cs.Insert(idx + 1, "        " + ss);
                        cs.Insert(idx + 1, "        " + s[i + 4]);
                        cs.Insert(idx + 1, "        " + s[i + 3]);
                        cs.Insert(idx + 1, "        " + s[i + 2]);
                        cs.Insert(idx + 1, "        " + s[i + 1]);
                        File.WriteAllLines(path, cs, Encoding.UTF8);
                    }
                    else
                    {
