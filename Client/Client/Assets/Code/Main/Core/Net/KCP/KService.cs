...
namespace Main
{
    ...
    public sealed class KService: AService
    {
        ...
        public uint TimeNow
        {
            get
            {
                return (uint) (Timer.ClientTime- this.startTime);
            }
        }
...
#region 回调方法
        ...
        private static void KcpLog(IntPtr bytes, int len, IntPtr kcp, IntPtr user)
        {
            try
            {
                ...
                Loger.Log(System.Text.Encoding.UTF8.GetString(logBuffer, 0, len));
            }
            catch (Exception e)
            {
                Loger.Error(e);
        ...
        public KService(ThreadSynchronizationContext threadSynchronizationContext, IPEndPoint ipEndPoint, ServiceType serviceType)
        {
            ...
            this.startTime = Timer.ClientTime;
        ...
        public void ChangeAddress(long id, IPEndPoint address)
        {
            ...
            Loger.Log($"channel change address: {id} {address}");
        ...
        private void Recv()
        {
            ...
            while (socket != null && this.socket.Available > 0)
            {
                ...
                try
                {
                    ...
                    switch (flag)
                    {
                        ...
                        case KcpProtocalType.ACK: // connect返回
                            ...
                            if (kChannel != null)
                            {
                                Loger.Log($"kservice ack: {kChannel.Id} {remoteConn} {localConn}");
                        ...
                        case KcpProtocalType.FIN: // 断开
                            ...
                            Loger.Log($"kservice recv fin: {kChannel.Id} {localConn} {remoteConn} {error}");
                ...
                {
                    Loger.Error($"kservice error: {flag} {remoteConn} {localConn}\n{e}");
                }
            }
        }
        ...
        protected override void Get(long id, IPEndPoint address)
        {
            ...
            {
                Loger.Error($"kservice get error: {id}\n{e}");
            }
        }
        ...
        public override void Remove(long id)
        {
            ...
            Loger.Log($"kservice remove channel: {id} {kChannel.LocalConn} {kChannel.RemoteConn}");
        ...
        private void Disconnect(uint localConn, uint remoteConn, int error, IPEndPoint address, int times)
        {
            ...
            {
                Loger.Error($"Disconnect error {localConn} {remoteConn} {error} {address} {e}");
            }

            Loger.Log($"channel send fin: {localConn} {remoteConn} {address} {error}");
        }
        ...
        {
            List<long> waitRemoveChannels = new List<long>();
            foreach (long channelId in this.waitConnectChannels.Keys)
            {
                ...
                if (kChannel == null)
                {
                    Loger.Error($"RemoveConnectTimeoutChannels not found kchannel: {channelId}");
                    continue;
                }
