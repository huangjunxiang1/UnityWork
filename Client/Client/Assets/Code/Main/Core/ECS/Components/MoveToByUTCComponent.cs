...
public class MoveToByUTCComponent : SComponent
{
    ...
    static void In(MoveToByUTCComponent a, TransformComponent b)
    {
        ...
        a._startTime = Game.Timer.utc - 1;
        a._endTime = Game.Timer.utc;
    }
    [UpdateSystem]
    static void Update(MoveToByUTCComponent a, TransformComponent b, KVComponent kv)
    {
        if (!a._isMoving) return;
        float lerp = (Game.Timer.utc - a._startTime) / (float)(a._endTime - a._startTime);
        ...
        if (lerp < 1)
        {
            ...
            b.rotation = math.slerp(b.rotation, r, math.clamp(Game.DeltaTime * kv.Get((int)KType.RotateSpeed), 0, 1));
        }
        else
        {
            b.rotation = math.slerp(b.rotation, a._r, math.clamp(Game.DeltaTime * kv.Get((int)KType.RotateSpeed), 0, 1));
