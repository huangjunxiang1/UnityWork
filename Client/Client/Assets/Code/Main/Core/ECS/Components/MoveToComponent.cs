...
public class MoveToComponent : SComponent
{
    ...
    static void Update(MoveToComponent a, TransformComponent b, KVComponent c)
    {
        ...
        if (a._style == MoveStyle.Linear)
        {
            ...
            float moveStep = Game.DeltaTime * speed;
            ...
            if (moveStep > 0)
            {
                ...
                b.rotation = math.slerp(b.rotation, r, math.clamp(Game.DeltaTime * speed2, 0, 1));
                b.position = now + dir * moveStep;
            }
            else
            {
                b.position = next;
                b.rotation = math.slerp(b.rotation, a.rotation, math.clamp(Game.DeltaTime * speed2, 0, 1));
        ...
        else if (a._style == MoveStyle.CatmullRom)
        {
            if (a._index < a._endIndex)
            {
                ...
                a._time += Game.DeltaTime / (math.distance(p1, p2) / speed);
                float t = a._time;
