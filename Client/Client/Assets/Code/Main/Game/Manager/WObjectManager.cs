...
namespace Game
{
    public class WObjectManager : ManagerM<WObjectManager>
    {
        long mySelfUnit;
        ...
        public void AddWObject(UnitInfo Unit)
        {
            ...
            if (role == null)
            {
                ...
                if (WRoot.Inst.GetChildren().Count > 1)
                {
                    CinemachineVirtualCamera cv = GameObject.FindObjectOfType<CinemachineVirtualCamera>();
                    cv.Follow = WRoot.Inst.GetChildren()[1].GameObject.transform;
                    cv.LookAt = WRoot.Inst.GetChildren()[1].GameObject.transform;
                }
            }
