...
partial class FUIFighting5
{
    ...
    protected override void OnEnter(params object[] data)
    {
        ...
        this._ip.text = this._serverLst.text;
        this._serverLst.onChanged.Add(() => this._ip.text = this._serverLst.text);
    }
    ...
    {
        var net = new STCP(Util.ToIPEndPoint(this._ip.text));
        NetComponent.Inst = Client.World.Root.AddComponent<NetComponent>().SetSession(net);
        await net.Connect();
        net.Work();
        connected = true;
        on_ref();
