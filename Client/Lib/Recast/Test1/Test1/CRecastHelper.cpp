...
dtNavMesh* loadMapOnData(const char* data)
{
	return loadMap(data);
	char* ptr = (char*)data;
	// Read header.
	...
	const dtPolyRef* visited, const int nvisited)
{
	...
	const int orig = furthestPath + 1 <= npath ? (furthestPath + 1) : npath;
	int size = npath - orig < 0 ? 0 : npath - orig;
...
bool CRecast::LoadMapOnData(const char* data)
{
	m_navMesh = loadMapOnData(data);
	m_navQuery = dtAllocNavMeshQuery();
	if (!m_navMesh)
		return false;
	m_navQuery->init(m_navMesh, 2048);
...
bool CRecastHelper::LoadMapOnData(int id, const char* data)
{
	CRecast* recast = new CRecast();
	bool ret = recast->LoadMapOnData(data);
	if (!ret)
		return false;
	m_mapRecast[id] = recast;
	return true;
