...
#region 网络线程
		...
		public override void Dispose()
		{
			...
			Loger.Log($"channel dispose: {this.Id} {this.RemoteAddress}");
		...
		private void StartRecv()
		{
			while (true)
			{
				...
				{
					Loger.Error($"tchannel error: {this.Id}\n{e}");
		...
		private void HandleRecv(object o)
		{
			...
			while (true)
			{
				...
				{
					Loger.Error($"ip: {this.RemoteAddress} {ee}");
		...
		private void OnRead(MemoryStream memoryStream)
		{
			...
			{
				Loger.Error($"{this.RemoteAddress} {memoryStream.Length} {e}");
		...
		{
			Loger.Log($"TChannel OnError: {error} {this.RemoteAddress}");
